...
import Query from '../components/QueryGQL.jsx';
import ArticleQuery from '../graphql/queries/ArticleQuery.jsx';
...
const PageArticle = () => {
	...
	return (
		<div>
			<Query query={ArticleQuery} slug={slug}>
				{({ data: { articles } }) => {
					UtilityTabTitle(`${articles.data[0].attributes.title} | Superior Software Solutions`);

					{
						/** date formatting start */
					}
					const publishedDate = `${articles.data[0].attributes.publishedAt}`;
					const formattedPublishedDate = format(parseISO(publishedDate), 'MMMM do, yyyy');
					{
						/** date formatting end */
					}
					...
					return (
						<div>
							...
							<section
								...
								style={{
									backgroundImage: `url(${articles.data[0].attributes.featured_image.data.attributes.url})`,
								}}
							>
								<div className="flex h-full w-full items-center justify-center bg-slate-900/50">
									<div className="text-center uppercase text-slate-100">
										<h1 className="mb-2 text-2xl font-semibold lg:text-3xl">{articles.data[0].attributes.title}</h1>
									</div>
								</div>
							</section>
							{/** hero section end */}
							...
							<section className="mx-auto max-w-3xl px-2 pt-8 pb-16">
								...
								<div className="flex items-center justify-between py-4 px-3">
									<div className="flex items-center space-x-3">
										<img
											...
											alt={articles.data[0].attributes.author.data.attributes.name}
											className="h-8 w-8 rounded-full"
										/>
										<div className="flex flex-col">
											<h4>{articles.data[0].attributes.author.data.attributes.name}</h4>
											<p className="text-sm">Published on {formattedPublishedDate}</p>
										</div>
									</div>
									<div className="flex items-center space-x-2.5">
										<p className="hidden text-sm font-medium tracking-wide lg:block">SHARE</p>
								...
								{/** content section - top author name plate end */}
								...
								{/** content section - bottom author name plate end */}
