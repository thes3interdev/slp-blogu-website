...
const NavigationHeader = () => {
	...
	return (
		<nav className="fixed inset-x-0 top-0 z-50 bg-sky-800">
			<div className="mx-auto max-w-7xl px-3 py-5">
				<div className="relative flex items-center justify-between">
					<Link to="/" className="inline-flex items-center">
						<span className="ml-2 text-xl font-semibold uppercase tracking-wide text-slate-100">Blogu</span>
					</Link>
							...
							<div className="absolute top-0 left-0 w-full">
								<div className="rounded border bg-slate-100 p-5 shadow-sm">
									<div className="mb-4 flex items-center justify-between">
										<div>
											<Link onClick={handleClose} to="/" className="inline-flex items-center">
												<span className="text-xl font-bold uppercase tracking-wide text-gray-800">Blogu</span>
											</Link>
										</div>
									...
									<div>
										<ul className="space-y-4">
											{navigationLinks.map((link) => (
												<li key={link.name}>
													<Link onClick={handleClose} to={link.link} className="text-lg font-semibold tracking-wider">
														{link.name}
													</Link>
												</li>
											))}
